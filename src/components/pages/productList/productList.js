...
import { addToList, addCopyCount, setAlertText,addControlList,addNumberList,inscreaseNumber,deleteControlList,deleteNumberList } from '../../../../Slice';
...
const ProductList = () => {
  ...
  return (
    <View>
      ...
      headerText={"Listem"} addButtonPressed={()=>setAddProductWindow(!addProductWindow)}  />
      <FlatList
        data={List}
        renderItem={({ item }) => {
          return (
            <FlatListItem  listDeleteIconVisible={listDeleteIcon} text={item.product} onPress={() => {
